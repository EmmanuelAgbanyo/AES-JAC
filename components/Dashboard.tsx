...
const Dashboard = ({ entrepreneurs, transactions }: DashboardProps) => {
    ...
    return (
        <div className="space-y-8 pb-10">
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 bg-white/40 dark:bg-black/20 backdrop-blur-md p-6 rounded-3xl border border-white/20 dark:border-white/5 shadow-xl shadow-indigo-500/5 mb-8 animate-slideUp relative overflow-hidden group">
                ...
                <div className="relative z-10">
                    <h1 className="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400 mb-2 tracking-tight">
                        Welcome back, <span className="text-aesBlue">Admin</span>
                    </h1>
                    <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">Here's what's happening with your entrepreneurs today.</p>
                </div>
                <div className="bg-white/60 dark:bg-white/5 p-1.5 rounded-2xl shadow-inner border border-white/20 dark:border-white/5 flex items-center backdrop-blur-sm relative z-10">
                    {DATE_RANGE_OPTIONS.map(opt => (
                        <button
                            key={opt.key}
                            onClick={() => setDateRange(opt.key)}
                            className={`px-4 py-2 text-sm font-bold rounded-xl transition-all duration-300 ${dateRange === opt.key
                                ? 'bg-white dark:bg-aesBlue text-aesBlue dark:text-white shadow-lg shadow-gray-200/50 dark:shadow-none scale-100'
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                {/* Chart Section */}
                <div className="xl:col-span-2 bg-white/60 dark:bg-black/20 backdrop-blur-2xl p-8 rounded-3xl shadow-xl border border-white/40 dark:border-white/5 hover:border-white/60 transition-colors duration-300 animate-slideUp" style={{ animationDelay: '200ms' }}>
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">Financial Performance</h3>
                        <button className="text-sm font-bold text-aesBlue hover:text-blue-700 transition-colors bg-blue-50 dark:bg-white/5 px-4 py-2 rounded-xl hover:bg-blue-100">Download Report</button>
                    </div>
                ...
                <div className="space-y-8 animate-slideUp" style={{ animationDelay: '300ms' }}>
                    ...
                    <div className="bg-white/60 dark:bg-black/20 backdrop-blur-2xl p-8 rounded-3xl shadow-xl border border-white/40 dark:border-white/5 hover:border-white/60 transition-colors duration-300">
                        ...
                        {topEntrepreneurs.length > 0 ? (
                            <ul className="space-y-3">
                                {topEntrepreneurs.map((e, index) => (
                                    <li key={index} className="flex justify-between items-center p-3 rounded-2xl hover:bg-white/40 dark:hover:bg-white/5 transition-all duration-300 border border-transparent hover:border-white/30 group">
                                        <div className="flex items-center gap-4">
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shadow-md transition-transform group-hover:scale-110 duration-300 ${index === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 text-white ring-2 ring-yellow-400/30' :
                                                index === 1 ? 'bg-gradient-to-br from-slate-300 to-slate-500 text-white' :
                                                    index === 2 ? 'bg-gradient-to-br from-amber-600 to-amber-700 text-white' :
                                                        'bg-white dark:bg-white/10 text-gray-600 dark:text-gray-300 border border-gray-100 dark:border-white/5'
                                                }`}>
                                                {index + 1}
                                            </div>
                                            <span className="font-semibold text-gray-700 dark:text-gray-200">{e.name}</span>
                                        </div>
                                        <span className="font-bold text-gray-900 dark:text-white bg-gray-50 dark:bg-white/5 px-2.5 py-1 rounded-lg border border-gray-100 dark:border-white/5">
